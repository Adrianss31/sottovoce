...
python3 scripts/sign_release.py \
  ...
  --version 0.6.1 --code 21 \
  --notes-file /percorso/note-release.md \
  --output /percorso/release-0.6.1
```
...
4. Crea una release in bozza con un tag corrispondente alla versione, per esempio `v0.6.1`, sul commit effettivamente verificato. Allega l’APK firmato, `update.json` e `SHA256SUMS`.
