...
python3 scripts/sign_release.py \
  ...
  --version 0.4.4 --code 17 \
  --notes-file /percorso/note-release.md \
  --output /percorso/release-0.4.4
```
...
4. Crea una release in bozza con un tag corrispondente alla versione, per esempio `v0.4.4`, sul commit effettivamente verificato. Allega l’APK firmato, `update.json` e `SHA256SUMS`.
