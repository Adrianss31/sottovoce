...
- Collega l’intera pagina, anziché la sola intestazione, alla tessera o al comando da cui è stata aperta: il ritorno resta l’esatto inverso anche dopo aver scorso molto la schermata.
- Aggiunge una prova automatica che apre un libro in fondo alla libreria, scorre fino alla gestione del libro e verifica che il ritorno mostri ancora il titolo di origine nella stessa zona.
- Esegue le prove Android con le animazioni abilitate, così la pipeline verifica davvero il codice delle transizioni condivise.

## Novità della 0.4.4

- Usa per libri, serie, mini-player e impostazioni lo stesso movimento reversibile già presente in “Il tuo ascolto”: il contenitore aperto si espande nella destinazione e torna esattamente al punto di origine.
- Ridisegna la libreria con tessere orizzontali più dense, copertine compatte, gerarchia tipografica più chiara e stato/progresso leggibili senza grandi spazi vuoti.
- Compatta la lista dei capitoli e aggiunge in ogni riga stato, durata e, per il capitolo corrente, tempo ascoltato, residuo e percentuale.
